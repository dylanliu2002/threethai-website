...
1. Read workspace and repository `AGENTS.md`, then the exact Task Card and Task
   Contract named by the controller. The contract cannot authorize itself.
2. Require a matching external Authorization Grant, signed controller
   capability, authoritative run, current lease and fencing token. Verify the
   controller-supplied `task_key`, branch, worktree, Role, routing and scope.
...
6. Run the granted validation profile and return only the required strict
   worker-result schema. Include exact base/head, changed files and evidence;
   the controller independently derives actual Git/filesystem changes.
7. Do not publish, merge, deploy, use secrets, take external action or alter
   production unless the external Grant, controller capability, live lease and
   current activation explicitly authorize that exact action.
