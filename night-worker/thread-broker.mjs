...
const GENUINE_BROKER_CLIENTS = new WeakMap();

export function isGenuineThreadBrokerForClient(broker, client, bindingToken) {
  const binding = GENUINE_BROKER_CLIENTS.get(broker);
  return binding?.client === client && binding?.token === bindingToken;
}
...
function assertExactSubmittedWorktree(root, candidate) {
  const canonicalRoot = assertGitWorktree(canonicalDirectory(root, "submitted repository root"), "submitted repository root");
  if (root !== canonicalRoot) throw new Error("Submitted repository root must be canonical.");
  const canonicalCandidate = assertGitWorktree(canonicalDirectory(candidate, "Worker cwd"), "Worker cwd");
  if (!sameCanonicalPath(canonicalRoot, canonicalCandidate)) {
    throw new Error("Worker cwd must be the exact submitted Git worktree.");
  }
  return canonicalCandidate;
}

function assertPersistedMappingCwd(root, mappingCwd) {
  const canonicalRoot = assertGitWorktree(canonicalDirectory(root, "submitted repository root"), "submitted repository root");
  if (root !== canonicalRoot) throw new Error("Persisted submitted repository root must be canonical.");
  const canonicalMappingCwd = assertGitWorktree(canonicalDirectory(mappingCwd, "Persisted worker cwd"), "Persisted worker cwd");
  if (!sameCanonicalPath(canonicalRoot, canonicalMappingCwd) || mappingCwd !== canonicalMappingCwd) {
    throw new Error("Persisted worker cwd must be the canonical exact submitted Git worktree.");
  }
  return canonicalMappingCwd;
}
...
export class ThreadBroker {
  ...
  constructor({
    ...
    const appServerClient = isGenuineAppServerClient(client);
    if (client instanceof AppServerClient && !appServerClient) {
      throw new Error("Thread Broker requires a genuine constructed App Server client.");
    }
    if (!appServerClient && (!client || typeof client.modelList !== "function")) {
    ...
    GENUINE_BROKER_CLIENTS.set(this, { client, token: bindingToken });
    this.#client = client;
    this.#authority = appServerClient ? Object.freeze({ broker: this, token: bindingToken }) : null;
    this.store = store;
    this.clock = clock;
  ...
  #callClient(method, ...args) {
    if (this.#authority) {
      return AppServerClient.prototype[method].call(this.#client, ...args, this.#authority);
    }
  ...
  async #validatePolicy(model, effort) {
    ...
    do {
      ...
      const page = await this.#callClient("modelList", params);
  ...
  async #recoverAmbiguousTurn({ batch, task, mapping, reservation, normalized, role, model, effort }) {
    ...
    assertPersistedMappingCwd(batch.repository_root, mapping.cwd);
    const available = await this.#validatePolicy(model, effort);
    const read = await this.#callClient("threadRead", mapping.thread_id, { includeTurns: true });
  ...
  async #startOnce(options, role, normalized, policy) {
    ...
    assertExactSubmittedWorktree(batch.repository_root, normalized.cwd);
    ...
    try {
      ...
      const started = await this.#callClient("threadStart", {
        model,
      ...
      leaseKeeper.assertOwned();
      const resumed = await this.#callClient("threadResume", mapping.thread_id, {
        model,
        modelProvider: MVP_CONFIG.model_provider,
        config: { model_reasoning_effort: effort },
        cwd: normalized.cwd,
      ...
      const turnResponse = await this.#callClient("turnStart", {
  ...
  async readWorker({ batchId, batch_id: batchIdAlias, taskId, task_id: taskIdAlias, role }) {
    ...
    return this.#callClient("threadRead", mapping.thread_id, { includeTurns: true });
  }
  ...
  #durableMapping(batchId, taskId, role) {
    ...
    if (mapping) assertPersistedMappingCwd(batch.repository_root, mapping.cwd);
