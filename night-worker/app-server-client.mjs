...
import { isGenuineThreadBrokerForClient } from "./thread-broker.mjs";
...
const GENUINE_APP_SERVER_CLIENTS = new WeakSet();
...
function assertBrokerLifecycleCapability(client, capability) {
  if (!isGenuineThreadBrokerForClient(capability?.broker, client, capability?.token)) {
...
export function isGenuineAppServerClient(client) {
  return GENUINE_APP_SERVER_CLIENTS.has(client);
}
...
export class AppServerClient extends EventEmitter {
  constructor(options = {}) {
    super();
    if (new.target !== AppServerClient) {
      throw new Error("App Server client subclasses are not permitted.");
    }
    ...
    GENUINE_APP_SERVER_CLIENTS.add(this);
  }
  ...
  threadStart(params = {}, capability) {
    assertBrokerLifecycleCapability(this, capability);
...
for (const method of ["modelList", "threadStart", "turnStart", "threadRead", "threadResume"]) {
  const descriptor = Object.getOwnPropertyDescriptor(AppServerClient.prototype, method);
  Object.defineProperty(AppServerClient.prototype, method, {
    ...descriptor,
    configurable: false,
    writable: false,
  });
