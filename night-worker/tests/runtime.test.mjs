...
import * as appServerClientModule from "../app-server-client.mjs";
...
async function spawnDeadProcess() {
...
function startLockWorker({ storePath, ownerToken, startBuffer, releaseBuffer, lockTimeoutMs = 250 }) {
  const source = `
    const { parentPort, workerData } = require("node:worker_threads");
    (async () => {
      const { RuntimeStore } = await import(workerData.runtimeModule);
      const start = new Int32Array(workerData.startBuffer);
      const release = new Int32Array(workerData.releaseBuffer);
      const store = new RuntimeStore(workerData.storePath, {
        lockTimeoutMs: workerData.lockTimeoutMs,
        lockOwnerTokenFactory: () => workerData.ownerToken,
      });
      parentPort.postMessage({ type: "ready" });
      Atomics.wait(start, 0, 0);
      try {
        store.mutate((state) => {
          parentPort.postMessage({ type: "acquired", ownerToken: workerData.ownerToken });
          Atomics.wait(release, 0, 0, 5000);
          return state;
        });
        parentPort.postMessage({ type: "success", ownerToken: workerData.ownerToken });
      } catch (error) {
  ...
  const worker = new Worker(source, {
    eval: true,
    workerData: {
      runtimeModule: new URL("../runtime-store.mjs", import.meta.url).href,
      storePath,
      ownerToken,
      startBuffer,
      releaseBuffer,
...
test("submission and broker reject non-Git roots, symlink escapes, and external CLI stores", async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      /exact submitted Git worktree/,
    );
    const nested = path.join(f.root, "nested-worktree");
    fs.mkdirSync(nested);
    execFileSync("git", ["init", "--quiet", nested], { stdio: "ignore" });
    await assert.rejects(
      broker.startImplementation({ batchId: batch.batch_id, taskId: batch.tasks[0].task_id, cwd: nested, prompt: "nested" }),
      /exact submitted Git worktree/,
    );
...
test("runtime lock publishes complete ownership and recovers dead owners exactly once", async () => {
  const f = fixture();
  ...
  try {
    ...
    assert.throws(
      () => new RuntimeStore(store.filePath, { processAliveFn: () => false }),
      /Runtime process liveness cannot be injected/,
    );
    ...
    const recoveryStore = new RuntimeStore(store.filePath, {
      lockTimeoutMs: 25,
      clock: f.clock,
      lockOwnerTokenFactory: () => "owner-recovered",
    });
    fs.mkdirSync(recoveryStore.lockPath);
    fs.writeFileSync(path.join(recoveryStore.lockPath, "owner.json"), JSON.stringify({
      schema_version: 1,
    ...
    recoveryStore.mutate((state) => state);
    assert.equal(fs.existsSync(recoveryStore.lockPath), false);

    const competingDeadPid = await spawnDeadProcess();
    fs.mkdirSync(store.lockPath);
    fs.writeFileSync(path.join(store.lockPath, "owner.json"), JSON.stringify({
      schema_version: 1,
      owner_token: "competing-dead-owner",
      pid: competingDeadPid,
      host: "test-host",
      created_at: "2000-01-01T00:00:00.000Z",
    }), "utf8");
    const startBuffer = new SharedArrayBuffer(4);
    const releaseBuffer = new SharedArrayBuffer(4);
    const lockWorkers = [
      startLockWorker({
        storePath: store.filePath,
        ownerToken: "competing-owner-a",
        startBuffer,
        releaseBuffer,
      }),
      startLockWorker({
        storePath: store.filePath,
        ownerToken: "competing-owner-b",
        startBuffer,
        releaseBuffer,
      }),
    ];
    try {
      await Promise.all(lockWorkers.map((entry) => entry.waitFor("ready")));
      const acquired = lockWorkers.map((entry) => entry.waitFor("acquired"));
      Atomics.store(new Int32Array(startBuffer), 0, 1);
      Atomics.notify(new Int32Array(startBuffer), 0, 2);
      const firstOwner = await Promise.race(acquired);
      const firstIndex = firstOwner.ownerToken === "competing-owner-a" ? 0 : 1;
      const secondIndex = firstIndex === 0 ? 1 : 0;
      const loserFailure = await lockWorkers[secondIndex].waitFor("failure");
      assert.match(loserFailure.message, /Runtime store is busy/);
      assert.equal(fs.existsSync(store.lockPath), true);
      Atomics.store(new Int32Array(releaseBuffer), 0, 1);
      Atomics.notify(new Int32Array(releaseBuffer), 0, 2);
      const winnerResult = await lockWorkers[firstIndex].waitFor("success");
      assert.equal(winnerResult.ownerToken, firstOwner.ownerToken);
      assert.equal(fs.existsSync(store.lockPath), false);
    } finally {
      Atomics.store(new Int32Array(releaseBuffer), 0, 1);
      Atomics.notify(new Int32Array(releaseBuffer), 0, 2);
      await Promise.all(lockWorkers.map((entry) => entry.worker.terminate()));
    }
...
test("broker rejects a persisted worker cwd outside the exact submitted Git worktree before recovery RPCs", async () => {
  const f = fixture();
  try {
    const batch = f.submit(["persisted cwd"]);
    const seedBroker = new ThreadBroker({ client: new FakeBrokerClient(), store: f.store, clock: f.clock });
    await seedBroker.startImplementation({
      batchId: batch.batch_id,
      taskId: batch.tasks[0].task_id,
      cwd: f.root,
      prompt: "persisted cwd",
    });
    const corrupted = f.store.load();
    corrupted.workers[0].cwd = process.env.SystemRoot ?? "C:\\Windows";
    fs.writeFileSync(f.store.filePath, JSON.stringify(corrupted), "utf8");
    ...
    await assert.rejects(() => broker.readWorker(recovery), /genuine Git worktree|exact submitted Git worktree/);
    await assert.rejects(() => broker.resumeWorker(recovery), /genuine Git worktree|exact submitted Git worktree/);
    assert.equal(client.calls.length, 0);
  } finally {
    f.cleanup();
  }
...
test("App Server JSONL client performs one handshake, correlates responses, streams notifications, and rejects on exit", async () => {
  ...
  try {
    ...
    const batch = f.submit(["jsonl read"]);
    ...
    const broker = new ThreadBroker({ client, store: f.store, clock: f.clock });
    assert.equal(broker.client, undefined);
    assert.equal(Object.prototype.hasOwnProperty.call(broker, "client"), false);
    ...
    const second = broker.readWorker({
      batchId: batch.batch_id,
      taskId: batch.tasks[0].task_id,
      role: "IMPLEMENTATION",
    });
...
test("App Server client rejects exec, ephemeral/fork paths, secrets, and fallback", async () => {
  const f = fixture();
  try {
    let spawnCalls = 0;
    ...
    assert.equal(appServerClientModule.createThreadBrokerClient, undefined);
    assert.equal(client.child, undefined);
    assert.equal(client.transport, undefined);
    assert.equal(client.input, undefined);
    assert.equal(client.output, undefined);
    assert.equal(client.stderr, undefined);
    assert.equal(client.bindThreadBroker, undefined);
    ...
    assert.equal(broker.client, undefined);
    assert.equal(Object.prototype.hasOwnProperty.call(broker, "client"), false);
    const forgedBroker = Object.create(ThreadBroker.prototype);
    assert.throws(() => client.threadStart({}, {
      broker: forgedBroker,
      token: Object.freeze({}),
    }), /reserved for Thread Broker/);
...
test("App Server lifecycle binding rejects subclass and wrapper capture attempts", () => {
  const f = fixture();
  try {
    let captured = null;
    class CapturingClient extends AppServerClient {
      bindThreadBroker(...args) {
        captured = super.bindThreadBroker(...args);
        return captured;
      }
    }
    assert.throws(
      () => new CapturingClient({ transport: new FakeTransport() }),
      /App Server client subclasses are not permitted/,
    );
    assert.equal(captured, null);

    const client = new AppServerClient({ transport: new FakeTransport() });
    const wrapper = new Proxy(client, {
      get(target, property, receiver) {
        if (["threadStart", "turnStart", "threadRead", "threadResume"].includes(property)) captured = property;
        return Reflect.get(target, property, receiver);
      },
    });
    assert.throws(
      () => new ThreadBroker({ client: wrapper, store: f.store, clock: f.clock }),
      /genuine constructed App Server client/,
    );
    assert.equal(captured, null);
    assert.equal(client.bindThreadBroker, undefined);
  } finally {
