...
export class RuntimeStore {
  constructor(filePath, options = {}) {
    if (!options || typeof options !== "object" || Array.isArray(options)) {
      throw new Error("Runtime store options must be an object.");
    }
    if (Object.prototype.hasOwnProperty.call(options, "processAliveFn")
      || Object.prototype.hasOwnProperty.call(options, "process_alive")
      || Object.prototype.hasOwnProperty.call(options, "livenessChecker")) {
      throw new Error("Runtime process liveness cannot be injected.");
    }
    const {
      lockTimeoutMs = 5_000,
      clock = Date,
    ...
    return this.mutate((state) => {
      ...
      if (reservationIndex >= 0) {
        ...
        if (turnStartState === "TURN_START_IN_FLIGHT") {
          if (!this.#isProcessAlive(existingReservation.owner_pid)) {
            return { status: "AMBIGUOUS", reservation: clone(existingReservation) };
          }
          return { status: "BUSY", reservation: clone(existingReservation) };
        }
        const ownerDead = !this.#isProcessAlive(existingReservation.owner_pid);
  ...
  #recoveryMarkerPath() {
    return `${this.lock_path}.recovery`;
  }

  #reclaimDeadRecoveryMarker() {
    const markerPath = this.#recoveryMarkerPath();
    if (!fs.existsSync(markerPath)) return false;
    let stat;
    try { stat = fs.lstatSync(markerPath); } catch { return false; }
    if (!stat.isDirectory()) return false;
    const ownerPath = path.join(markerPath, "owner.json");
    let record;
    try { record = JSON.parse(fs.readFileSync(ownerPath, "utf8")); } catch { return false; }
    if (!record || record.schema_version !== RUNTIME_SCHEMA_VERSION
      || typeof record.owner_token !== "string" || record.owner_token.length === 0
      || !Number.isInteger(record.pid) || record.pid <= 0) return false;
    try { parsePersistedTimestamp(record.created_at, "Runtime recovery marker created_at"); } catch { return false; }
    if (this.#isProcessAlive(record.pid)) return false;
    const quarantinePath = `${markerPath}.stale.${process.pid}.${crypto.randomUUID()}`;
    try {
      fs.renameSync(markerPath, quarantinePath);
    } catch (error) {
      if (["ENOENT", "ENOTEMPTY", "EEXIST", "EISDIR", "EPERM"].includes(error?.code)) return false;
      throw error;
    }
    try {
      const movedOwnerPath = path.join(quarantinePath, "owner.json");
      const movedEntries = fs.readdirSync(quarantinePath);
      if (movedEntries.length !== 1 || movedEntries[0] !== "owner.json") {
        throw new Error("Runtime recovery marker has an unexpected owner directory layout.");
      }
      const movedRecord = JSON.parse(fs.readFileSync(movedOwnerPath, "utf8"));
      if (!movedRecord || movedRecord.schema_version !== RUNTIME_SCHEMA_VERSION
        || typeof movedRecord.owner_token !== "string" || movedRecord.owner_token.length === 0
        || !Number.isInteger(movedRecord.pid) || movedRecord.pid <= 0) {
        throw new Error("Runtime recovery marker has an invalid owner record.");
      }
      parsePersistedTimestamp(movedRecord.created_at, "Runtime recovery marker created_at");
      if (this.#isProcessAlive(movedRecord.pid)) {
        fs.renameSync(quarantinePath, markerPath);
        return false;
      }
      fs.unlinkSync(movedOwnerPath);
      fs.rmdirSync(quarantinePath);
      return true;
    } catch (error) {
      if (fs.existsSync(quarantinePath) && !fs.existsSync(markerPath)) {
        try { fs.renameSync(quarantinePath, markerPath); } catch {}
      }
      throw error;
    }
  }

  #tryAcquireRecoveryMarker() {
    const markerPath = this.#recoveryMarkerPath();
    this.#reclaimDeadRecoveryMarker();
    const ownerToken = `recovery_${process.pid}_${crypto.randomUUID()}`;
    const temporaryDirectory = `${markerPath}.${process.pid}.${crypto.randomUUID()}.tmp`;
    ...
    try {
      ...
      fs.renameSync(temporaryDirectory, markerPath);
      return { owner_token: ownerToken, path: markerPath };
    } catch (error) {
      try {
        if (fs.existsSync(temporaryDirectory)) fs.rmSync(temporaryDirectory, { recursive: true, force: true });
      } catch {}
      if (["EEXIST", "ENOTEMPTY", "EISDIR", "EPERM"].includes(error?.code)) return null;
      throw error;
    }
  }

  #releaseRecoveryMarker(marker) {
    const ownerPath = path.join(marker.path, "owner.json");
    let record;
    try { record = JSON.parse(fs.readFileSync(ownerPath, "utf8")); } catch (error) {
      throw new Error(`Runtime recovery marker ownership could not be verified: ${error instanceof Error ? error.message : String(error)}`);
    }
    if (!record || record.owner_token !== marker.owner_token) {
      throw new Error("Runtime recovery marker owner token does not match.");
    }
    fs.unlinkSync(ownerPath);
    fs.rmdirSync(marker.path);
  }

  #recoverDeadOwnerLock() {
    if (!fs.existsSync(this.lock_path)) return false;
    let stat;
    try { stat = fs.lstatSync(this.lock_path); } catch { return false; }
    if (!stat.isDirectory()) return false;
    ...
    try { parsePersistedTimestamp(record.created_at, "Runtime lock created_at"); } catch { return false; }
    let alive;
    try { alive = this.#isProcessAlive(record.pid); } catch { return false; }
    if (alive) return false;
    const marker = this.#tryAcquireRecoveryMarker();
    if (!marker) return false;
    try {
      const currentOwnerPath = path.join(this.lock_path, "owner.json");
      let currentRecord;
      try { currentRecord = JSON.parse(fs.readFileSync(currentOwnerPath, "utf8")); } catch { return false; }
      if (!currentRecord || currentRecord.schema_version !== RUNTIME_SCHEMA_VERSION
        || currentRecord.owner_token !== record.owner_token
        || currentRecord.pid !== record.pid
        || currentRecord.created_at !== record.created_at) return false;
      if (this.#isProcessAlive(currentRecord.pid)) return false;
      const lockEntries = fs.readdirSync(this.lock_path);
      if (lockEntries.length !== 1 || lockEntries[0] !== "owner.json") return false;
      fs.unlinkSync(currentOwnerPath);
      fs.rmdirSync(this.lock_path);
      return true;
    } catch (error) {
      throw error;
    } finally {
      this.#releaseRecoveryMarker(marker);
    }
  }
  ...
  #withLock(callback) {
    ...
    while (!owner) {
      ...
      try {
        ...
        if (this.#recoverDeadOwnerLock()) continue;
