  ...
  return (
    <>
      {jsonLd([breadcrumbSchema([
        ...
        { name: dict.breadcrumbs.applications, path: "/applications" },
      ])])}
      <section className="border-b border-border bg-primary text-primary-foreground">
        <div className="container-site py-12 sm:py-16">
          ...
          <p className="eyebrow-light mt-6">Applications</p>
        ...
        <div className="container-site grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <Reveal key={app.slug} delay={index * 60} className="h-full">
              ...
              >
                <div className="flex flex-1 flex-col p-6">
                  ...
                  <h2 className="mt-2 text-lg font-semibold text-ink group-hover:text-primary">{app.name.en}</h2>
                  <p className="mt-2 flex-1 text-sm leading-relaxed text-muted-foreground">{app.summary.en}</p>
                ...
                <ul className="border-t border-border bg-paper/60 px-6 py-4">
                  {app.productSlugs.slice(0, 2).map((slug) => (
                    <li key={slug} className="text-xs text-muted-foreground">
                      → {en.nav.products}: {slug.replaceAll("-", " ")}
                    </li>
                  ))}
                </ul>
              </Link>
          ...
          <Reveal delay={applications.length * 60} className="h-full">
            <div className="flex h-full flex-col justify-between rounded-lg border border-primary/20 bg-secondary p-6">
              ...
              <Link href="/product-finder" className="btn-primary mt-5 w-fit !min-h-10 !px-4 text-sm">
