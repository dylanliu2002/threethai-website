...
export default function HomeKnowledge({ locale, dict }: { locale: Locale; dict: Dictionary }) {
  ...
  // The article headline and summary quoted here are this page's prose, not a claim
  // about /knowledge/<slug>: the article bodies stay untranslated and those pages
  // stay English-owner fallbacks. See src/content/card-copy.ts.
          ...
          {featured.map((article, index) => (
            <Reveal as="li" key={article.slug} delay={index * 70} className="h-full">
              ...
              >
                ...
                <h3 className="mt-3 flex-1 font-semibold leading-snug text-ink group-hover:text-primary">{teaser(article.slug).title}</h3>
                <p className="mt-3 line-clamp-2 text-sm leading-relaxed text-muted-foreground">{teaser(article.slug).intro}</p>
