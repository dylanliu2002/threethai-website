...
- [x] Validation recorded below; the human-authorized extra correction cycle is rebuilt on the current `origin/main` tip and remains pending independent SOL re-review.

Validation results so far:

- `node --test night-worker/tests/*.test.mjs` — PASS (32/32), including crash-window turn reconciliation, actual-ThreadBroker-only lifecycle binding, exact submitted-worktree recovery confinement, and dead/live runtime-lock contention tests.
- `npm run lint` — PASS.
...
- Extra correction evidence: `AppServerClient` keeps process/transport handles private, binds lifecycle access only to the actual `ThreadBroker` class with no public registration hook, requires the canonical submitted Git worktree for start/read/resume, and recovers a published lock only when its recorded owner process is proven dead; deterministic tests cover each boundary.
...
- Outcome: CHANGES_REQUESTED from fresh independent SOL review; the human-authorized extra correction cycle is implemented and pending fresh re-review.
- Independent reviewer evidence:
  ...
  - Extra-cycle findings addressed: actual-ThreadBroker-only lifecycle binding with private process handles, exact canonical submitted-worktree enforcement before start/read/resume, and dead-owner-only runtime-lock recovery with live-owner contention coverage.

## Completion Record

- Commit: `bb49ea387285bc9d6283990455d62e0b0e7399c6` (extra correction lifecycle-authority hardening tip, linearly descended from the current `origin/main`-based Task 61 delivery)
- Handoff evidence commit: the post-hardening Task 61 card/worklog descendant recorded in the append-only worklog below
...
- Validation results: focused tests PASS (32/32); `npm run lint` PASS; local `npm run build` is blocked because the restricted environment cannot fetch Google Geist and Geist Mono from Google Fonts; independent SOL evidence records a clean build PASS for reviewed PR head `b9902288b4bf81781841c7fe4232b3b6ff63b207`; implementation and final handoff diff/check, committed paths, package/workflow immutability, `origin/main` ancestry, and exact identity gates are recorded for the new extra-correction tip and its card/worklog descendant.
- Worklog: `worklog/agent-61-night-worker-runtime.md`
...
## 2026-09-13 — final targeted correction evidence

- Addressed the two persisted blockers within the Task 61 allowlist: only genuinely constructed brokers associated with the exact App Server client can receive the private lifecycle channel, and the channel is no longer reachable through `broker.client`; runtime liveness is a private real-process decision, while dead-owner recovery is serialized by an atomic recovery marker and revalidates the exact owner before replacement.
- Focused regressions pass 2/2, repeated three times; full `node --test night-worker/tests/*.test.mjs` passes 32/32; `npm run lint` passes. The prior environment-only Google Fonts build blocker is unchanged and no build/font/shared file was touched in this correction.
- Targeted source commit: `4a9299f22a2ae2a20c73b47bd8fa481b21da472e`, parent `4f3488c021180e585009686a849fd3a59c13167b`; final card/worklog handoff descendant is recorded in the task-owned worklog. Status remains `REVIEW` pending fresh independent SOL review.

## 2026-09-14 — final lifecycle correction and fresh-base evidence

- Closed the remaining lifecycle-authority blocker: `AppServerClient.bindThreadBroker()` and the lifecycle facade factory are removed from the public surface; ThreadBroker keeps the raw client and opaque authority object private, dispatches through the immutable base lifecycle methods, and rejects subclass/proxy clients and forged broker authority before any lifecycle request.
- Focused lifecycle regressions pass 2/2; full `node --test night-worker/tests/*.test.mjs` passes 33/33; `npm run lint` passes. Existing mutex regressions remain included and passing.
- Fresh-base rebuild target is `origin/main` `14a658961855f8e0b5cdce6a3c63867ff95dc51b`; the final delivery lineage and hash-qualified gates are recorded in the task-owned worklog. Status remains `REVIEW` pending fresh independent SOL review.

## 2026-09-14 — fresh-main lifecycle handoff evidence
